...
tavily_client = TavilyClient(api_key='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')
...
def getOutPutInFormat(model: genai.GenerativeModel, query: str, image_names: List[str], output_type, versionNew: bool = False):
    try:
        print(f"\n=== getOutPutInFormat ===")
        print(f"Output type requested: {output_type}")
        ...
        # Special handling for List[str] output type
        if output_type == List[str]:
            print("Using special handling for List[str] output type")
            modified_query = query + "\nPlease respond with a JSON array of strings."
            print(f"Modified query: {modified_query}")
            
            response = model.generate_content(
                [modified_query] + images_opened,
                generation_config=genai.GenerationConfig(
                    response_mime_type="application/json"
                ),
            )
        else:
            print("Using standard output type handling")
            response = model.generate_content(
                [query] + images_opened,
                generation_config=genai.GenerationConfig(
                    response_mime_type="application/json", 
        ...
        result = responseGemini(response) if versionNew else response.text
        print(f"Generated response: {result}")
        return result
    except Exception as e:
        print(f"Error in getOutPutInFormat: {str(e)}")
...
def typeDocInputNOutputFormat(model: genai.GenerativeModel, query: str, output_type, report_path: str):
    ...
    try:
        ...
        print(f"\nFile processing complete. State: {video_file.state.name}")
        ...
        print(f"Error in typeDocInputNOutputFormat: {str(e)}")
        print(f"Error type: {type(e)}")
        import traceback
        print(f"Traceback: {traceback.format_exc()}")
        return {"error": str(e)}
